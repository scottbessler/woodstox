		...
		InputConfigFlags, ParsingErrorMsgs
{
	 ...
	 * and > (">" only matters when quoting text, as part of "]]>")
	 */
	protected final static char CHAR_FIRST_PURE_TEXT = (char) ('>' + 1);

	/**
	 * First character in Unicode (ie one with lowest id) that is legal as part of a local name (all valid name chars minus ':'). Used for doing quick check for
	 * local name end; usually name ends in a whitespace or equals sign.
	 */
	...
	/**
	 * Local full name for the event, if it has one (note: element events do NOT use this variable; those names are stored in element stack): target for
	...
	/**
	 * Top-most input source this reader can use; due to input source chaining, this is not necessarily the root of all input; for example, external DTD subset
	 * reader's root input still has original document input as its parent.
	 */
	...
	/**
	 * This is the current depth of the input stack (same as what input element stack would return as its depth). It is used to enforce input scope constraints
	 * for nesting of elements (for xml reader) and dtd declaration (for dtd reader) with regards to input block (entity expansion) boundaries.
	 * <p>
	 * Basically this value is compared to {@link #mInputTopDepth}, which indicates what was the depth at the point where the currently active input scope/block
	 * was started.
	 */
	...
	/**
	 * Flag that indicates whether linefeeds in the input data are to be normalized or not. Xml specs mandate that the line feeds are only normalized when they
	 * are from the external entities (main doc, external general/parsed entities), so normalization has to be suppressed when expanding internal general/parsed
	 * entities.
	 */
	...
	/**
	 * Total number of characters read before start of current token. For big (gigabyte-sized) sizes are possible, needs to be long, unlike pointers and sizes
	 * related to in-memory buffers.
	 */
	...
	/**
	 * XML version as declared by the document; one of constants from {@link XmlConsts} (like {@link XmlConsts#XML_V_10}).
	 */
	...
	/**
	 * Returns location of last properly parsed token; as per StAX specs, apparently needs to be the end of current event, which is the same as the start of the
	 * following event (or EOF if that's next).
	 */
	@Override
	...
	/**
	 * <p>
	 * Note: this is the base implementation used for implementing <code>ValidationContext</code>
	 */
	@Override
	...
	/**
	 * Construct and return a {@link XMLStreamException} to throw as a result of a failed Typed Access operation (but one not caused by a Well-Formedness
	 * Constraint or Validation Constraint problem)
	 */
	/*
	...
	*/
	...
	/**
	 * Similar to {@link #throwUnexpectedEOF}, but only indicates ending of an input block. Used when reading a token that can not span input block boundaries
	 * (ie. can not continue past end of an entity expansion).
	 */
	 ...
	 * <p>
	 * Note: this method only peeks within current input source; it does not close it and check nested input source (if any). This is necessary when checking
	 * keywords, since they can never cross input block boundary.
	 */
	...
	/**
	 * Similar to {@link #getNextChar}, but will not read more characters from parent input source(s) if the current input source doesn't have more content.
	 * This is often needed to prevent "runaway" content, such as comments that start in an entity but do not have matching close marker inside entity; XML
	 * specification specifically states such markup is not legal.
	 */
	...
	/**
	 * Method that will skip through zero or more white space characters, and return either the character following white space, or -1 to indicate EOF (end of
	 * the outermost input source)/
	 */
	...
	/**
	 * Method called when a CR has been spotted in input; checks if next char is LF, and if so, skips it. Note that next character has to come from the current
	 * input source, to qualify; it can never come from another (nested) input source.
	 * 
	...
	/**
	 * Method called when an entity has been expanded (new input source has been created). Needs to initialize location information and change active input
	 * source.
	 * 
	 * @param entityId
	 *            Name of the entity being expanded
	 */
	 ...
	 * 
	 * @return true if reading succeeded (or may succeed), false if we reached EOF.
	 */
	 ...
	 * {@link #loadMore}. It can only be called when input comes from main-level buffer; further, call can shift content in input buffer, so caller has to flush
	 * any data still pending. In short, caller has to know exactly what it's doing. :-)
	 * <p>
	...
	protected void throwNullParent(WstxInputSource curr)
	{
		...
		// throw new
		// IllegalStateException("Internal error: null parent for input source '"+curr+"'; should never occur (should have stopped at root input '"+mRootInput+"').");
	}
	...
	/**
	 * Method that tries to resolve a character entity, or (if caller so specifies), a pre-defined internal entity (lt, gt, amp, apos, quot). It will succeed
	 * iff:
	 * <ol>
	 * <li>Entity in question is a simple character entity (either one of 5 pre-defined ones, or using decimal/hex notation), AND
	 * <li>
	 ...
	 * 
	 * @param checkStd
	 *            If true, will check pre-defined internal entities (gt, lt, amp, apos, quot); if false, will only check actual character entities.
	 * 
	 * @return (Valid) character value, if entity is a character reference, and could be resolved from current input buffer (does not span buffer boundary);
	 *         null char (code 0) if not (either non-char entity, or spans input buffer boundary).
	 */
	...
	/**
	 * Method called to resolve character entities, and only character entities (except that pre-defined char entities -- amp, apos, lt, gt, quote -- MAY be
	 * "char entities" in this sense, depending on arguments). Otherwise it is to return the null char; if so, the input pointer will point to the same point as
	 * when method entered (char after ampersand), plus the ampersand itself is guaranteed to be in the input buffer (so caller can just push it back if
	 * necessary).
	 * <p>
	 ...
	 * <p>
	 * Main complication here is that we need to do 5-char lookahead. This is problematic if chars are on input buffer boundary. This is ok for the root level
	 * input buffer, but not for some nested buffers. However, according to XML specs, such split entities are actually illegal... so we can throw an exception
	 * in those cases.
	 * 
	 * @param checkStd
	 *            If true, will check pre-defined internal entities (gt, lt, amp, apos, quot) as character entities; if false, will only check actual 'real'
	 *            character entities.
	 * 
	 * @return (Valid) character value, if entity is a character reference, and could be resolved from current input buffer (does not span buffer boundary);
	 *         null char (code 0) if not (either non-char entity, or spans input buffer boundary).
	...
	/**
	 * Reverse of {@link #resolveCharOnlyEntity}; will only resolve entity if it is NOT a character entity (or pre-defined 'generic' entity; amp, apos, lt, gt
	 * or quot). Only used in cases where entities are to be separately returned unexpanded (in non-entity-replacing mode); which means it's never called from
	 * dtd handler.
	 */
	...
	/**
	 * Method that does full resolution of an entity reference, be it character entity, internal entity or external entity, including updating of input buffers,
	 * and depending on whether result is a character entity (or one of 5 pre-defined entities), returns char in question, or null character (code 0) to
	 * indicate it had to change input source.
	 * 
	 * @param allowExt
	 *            If true, is allowed to expand external entities (expanding text); if false, is not (expanding attribute value).
	 * 
	 * @return Either single-character replacement (which is NOT to be reparsed), or null char (0) to indicate expansion is done via input source.
	 */
	 ...
	 *            Name of the entity being expanded
	 * @param allowExt
	 *            Whether external entities can be expanded or not; if not, and the entity to expand would be external one, an exception will be thrown
	 */
	 ...
	 * @param allowExt
	 *            Whether external entities are allowed or not.
	 */
			...
			throws XMLStreamException
	{
		...
		if (isExt) {
			if (!allowExt) { // never ok in attribute value...
				throwParseError("Encountered a reference to external parsed entity \"{0}\" when expanding attribute value: not legal as per XML 1.0/1.1 #3.1",
						id, null);
			}
	 ...
	 * @param arg
	 *            Optional argument passed from caller; needed by DTD reader.
	 */
	protected abstract EntityDecl findEntity(String id, Object arg)
			throws XMLStreamException;

	/**
	 * This method gets called if a declaration for an entity was not found in entity expanding mode (enabled by default for xml reader, always enabled for dtd
	 * reader).
	 */
	 ...
	 * <p>
	 * Much of complexity in this method has to do with the intention to try to avoid any character copies. In this optimal case algorithm would be fairly
	 * simple. However, this only works if all data is already in input buffer... if not, copy has to be made halfway through parsing, and that complicates
	 * things.
	 * <p>
	 * One thing to note is that String returned has been canonicalized and (if necessary) added to symbol table. It can thus be compared against other such
	 * (usually id) Strings, with simple equality operator.
	 * 
	 * @param c
	 *            First character of the name; not yet checked for validity
	 * 
	 * @return Canonicalized name String (which may have length 0, if EOF or non-name-start char encountered)
	 */
	...
	/**
	 * Second part of name token parsing; called when name can continue past input buffer end (so only part was read before calling this method to read the
	 * rest).
	 * <p>
	 ...
	 * no namespace prefix (PI target, entity/notation name); if not, name can contain arbitrary number of colons. Note that element and attribute names are NOT
	 * parsed here, so actual namespace prefix separation can be handled properly there.
	 * <p>
	 * Similar to {@link #parseLocalName}, much of complexity stems from trying to avoid copying name characters from input buffer.
	 * <p>
	 * Note that returned String will be canonicalized, similar to {@link #parseLocalName}, but without separating prefix/local name.
	 * 
	 * @return Canonicalized name String (which may have length 0, if EOF or non-name-start char encountered)
	 */
	 ...
	 * Won't do any further validations, and parsing is not optimized: main need is just to get more meaningful error messages.
	 */
	...
	/**
	 * Note: does not check for number of colons, amongst other things. Main idea is to skip through what superficially seems like a valid id, nothing more.
	 * This is only done when really skipping through something we do not care about at all: not even whether names/ids would be valid (for example, when
	 * ignoring internal DTD subset).
	 * 
	 ...
	 * <p>
	 * NOTE: returned String is not canonicalized, on assumption that external ids may be longish, and are not shared all that often, as they are generally just
	 * used for resolving paths, if anything. <br />
			...
			throws XMLStreamException
	{
		...
		if (value >= 0xD800) {
			...
			if (!mXml11 && (value != 0x9 && value != 0xA && value != 0xD)) {
				return;
				// reportIllegalChar(value);
			}
		}
	...
	/**
	 * Method called to throw an exception indicating that a name that should not be namespace-qualified (PI target, entity/notation name) is one, and reader is
	 * namespace aware.
	 */
